...
class PlotSelectorDialog(QDialog):
    ...
    def _build(self):
        ...
        if is_adding:
            sub = QLabel(
                f"Already computed: {len(self._already_computed)} field(s)\n"
                "Cyan = already in store (instant).  White = will run Farneback again."
            )
            sub.setStyleSheet(f"color:{C_MUTED}; font-size:9px;")
        ...
        for gname, keys in groups:
            box = QGroupBox(gname)
            gl  = QVBoxLayout(box); gl.setSpacing(4)
            for key in keys:
                ...
                if already:
                    ...
                    cb.setToolTip("Not yet computed — will be computed on confirm")
        ...
        ba.clicked.connect(lambda:[c.setChecked(True)  for c in self._checks.values()])
        bn.clicked.connect(lambda:[c.setChecked(False) for c in self._checks.values()])
        br.addWidget(ba); br.addWidget(bn)
        layout.addLayout(br)

        is_adding = bool(self._already_computed)
        ok_label  = "ADD  APPLY CHANGES" if is_adding else "▶  COMPUTE & DISPLAY"
...
class ProgressDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.sub = QLabel("")
        self.sub.setStyleSheet(f"color:{C_MUTED}; font-size:10px;")
        layout.addWidget(self.sub)

    def update_progress(self, cur, tot):
        self.bar.setMaximum(tot); self.bar.setValue(cur)
        self.sub.setText(f"Frame {cur} / {tot}")
        self.label.setText(f"Computing fields...  {int(cur/tot*100)}%")
...
class SaveDialog(QDialog):
    ...
    def _build(self):
        ...
        # Export group
        eg = QGroupBox("EXPORT  (human-readable)")
        el = QVBoxLayout(eg); el.setSpacing(6)

        er = QWidget(); eh = QHBoxLayout(er); eh.setContentsMargins(0,0,0,0)
        btn_csv = QPushButton("Export full CSV (per pixel)")
        btn_npy = QPushButton("Export full npy arrays")
    ...
    def get_params(self):
        ...
        return self._action, path, mode, n, custom
...
class DICPlayerWindow(QMainWindow):
    ...
    def _add_fields(self):
        ...
        already_selected = set(self.selected_keys)
        ...
        def on_done():
            ...
            # Merge new keys into store
            merged = list(dict.fromkeys(
                list(self.store.selected_keys) + new_keys))
            self.store.selected_keys = merged
            for k, v in vmaxes_accum.items():
                self.store._vmaxes[k] = v
            print(f"[AddFields] done. keys={merged}  vmaxes={vmaxes_accum}")
    ...
    def _open_save_dialog(self):
        ...
        if dlg.result() != QDialog.Accepted:
            return
        action, path, mode, n, custom = dlg.get_params()
...
def run_dense_dic(frame_paths, polygon, mask, bbox, frame_step, fps,
    ...
    # Virtual keys (quiver) don't go to ComputeThread — derived at display time
    compute_keys   = [k for k in selected_keys if k not in VIRTUAL_KEYS]
    # Quiver needs vx+vy — auto-add them if quiver selected but not vx/vy
    if "quiver" in selected_keys:
        for dep in QUIVER_DEPS:
            if dep not in compute_keys:
                compute_keys.append(dep)

    # Temp HDF5 path for this session
    h5_path = make_temp_h5()

    # Progress + compute — writes directly to HDF5, nothing in RAM
    progress_dlg = ProgressDialog()
    progress_dlg.show()
    app.processEvents()

    thread = ComputeThread(frame_paths, polygon, mask, bbox,
                           frame_step, fps, fb_params, compute_keys,
    ...
    # Wait for compute using a QEventLoop tied to done/error signals
    from PyQt5.QtCore import QEventLoop
    compute_loop = QEventLoop()
    thread.done.connect(compute_loop.quit,  Qt.QueuedConnection)
    thread.error.connect(compute_loop.quit, Qt.QueuedConnection)
    # on_done/on_error already connected above — loop quits after they run
    compute_loop.exec_()
    thread.wait()
    ...
    # Player reads from HDF5 on demand — only WINDOW_SIZE frames in RAM
