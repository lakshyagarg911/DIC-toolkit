...
def run_icgn(
    ...
    for _it in range(max_iter):
        ...
        # ---- Convergence check ------------------------------------------
        # Radius-weighted convergence (translation px vs gradient dimensionless)
        r = float(max(abs(subset.dx.max()), 1))
        weighted_norm = np.sqrt(
            delta_p[0]**2 + delta_p[1]**2 +
            (r*delta_p[2])**2 + (r*delta_p[3])**2 +
            (r*delta_p[4])**2 + (r*delta_p[5])**2
        )
        if weighted_norm < conv_tol:
            converged = True
            break
