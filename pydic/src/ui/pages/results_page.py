...
CMAPS = ["viridis","plasma","inferno","magma","hot","cool","jet","turbo",
         "RdBu_r","seismic","bwr","coolwarm","Spectral_r","gray"]
...
class ResultsPage(QWidget):
    ...
    def _apply_overlay(self, arr: np.ndarray) -> None:
        ...
        try:
            ...
            rgba[~np.isfinite(arr), 3] = 0            # transparent where invalid
            self._canvas.set_result_overlay_rgba(rgba)

            # Update colorbar
            n_bar = 32
            bar_colors = [
                tuple(int(c * 255) for c in cmap_obj(i / (n_bar - 1))[:3])
